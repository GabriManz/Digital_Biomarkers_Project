...
RESULTS_DIR = PROJECT_ROOT / "outputs" / "results" / "transfer_learning"
FIGURES_DIR = PROJECT_ROOT / "outputs" / "figures" / "transfer_learning"
...
def run_loso_evaluation():
    ...
    xgb_pretrained = None
    if XGB_AVAILABLE:
        xgb_path = RESULTS_DIR / "xgb_pretrained.pkl"
        if xgb_path.exists():
            with open(xgb_path, "rb") as f:
                xgb_pretrained = pickle.load(f)
            print("Modelo XGBoost preentrenado cargado.")
            
    # Inicializar contenedores de resultados
    results = {
        "XGB_Transfer": {"y_true": [], "y_prob": [], "y_pred": [], "segs_auc": []},
        "CNN_Transfer": {"y_true": [], "y_prob": [], "y_pred": [], "segs_auc": []},
        "Ensemble_Transfer": {"y_true": [], "y_prob": [], "y_pred": [], "segs_auc": []}
    }
    
    # Si PyTorch está disponible, cargamos espectrogramas para CNN
    X_spectro = None
    if TORCH_AVAILABLE:
    ...
    print("\nEjecutando LOSO con Transfer Learning...")
    ...
    for fold_i, (train_idx, test_idx) in enumerate(loso.split(X_labeled, y_labeled, groups_labeled), start=1):
        ...
        if XGB_AVAILABLE and xgb_pretrained is not None:
            ...
            # Fine-tuning: entrenar más árboles usando el modelo preentrenado como base
            # Nota: para que xgb_model funcione en fit, el número de características debe coincidir.
            # Por tanto, si preentrenamos con 141 features, el selector debe desactivarse o entrenar sin selector para XGB.
            # Vamos a entrenar XGBoost directamente sobre las 141 características (sin SelectKBest)
            ...
            # built-in continuation: pass xgb_pretrained to fit
            xgb_fold.fit(X_train_xgb, y_train, xgb_model=xgb_pretrained.get_booster())
            
            y_prob_xgb = xgb_fold.predict_proba(X_test_xgb)[:, 1]
            y_pred_xgb = xgb_fold.predict(X_test_xgb)
            
            results["XGB_Transfer"]["y_prob"].extend(y_prob_xgb)
            results["XGB_Transfer"]["y_pred"].extend(y_pred_xgb)
            probs_fold["XGB_Transfer"] = y_prob_xgb
            
            auc_seg = float(roc_auc_score(y_test, y_prob_xgb)) if len(np.unique(y_test)) > 1 else 0.5
            results["XGB_Transfer"]["segs_auc"].append(auc_seg)
            
        # --- B. CNN con transferencia (Fine-tuning) ---
        if TORCH_AVAILABLE and X_spectro is not None:
            device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
            cnn_pretrained_path = RESULTS_DIR / "cnn_pretrained.pt"
            ...
            # Fine-tuning rápido con learning rate menor
            ...
            model.train()
            for epoch in range(5):  # Pocas épocas para fine-tuning local rápido
                for Xb, yb in dl:
            ...
            model.eval()
            X_te_spec = torch.tensor(X_spectro[test_idx], dtype=torch.float32).unsqueeze(1).to(device)
            with torch.no_grad():
                logits_te = model(X_te_spec)
                probs_te = torch.sigmoid(logits_te).cpu().numpy()
            preds_te = (probs_te >= 0.5).astype(int)
            
            results["CNN_Transfer"]["y_prob"].extend(probs_te)
            results["CNN_Transfer"]["y_pred"].extend(preds_te)
            probs_fold["CNN_Transfer"] = probs_te
            
            auc_seg = float(roc_auc_score(y_test, probs_te)) if len(np.unique(y_test)) > 1 else 0.5
            results["CNN_Transfer"]["segs_auc"].append(auc_seg)
            
        # --- C. Ensemble ---
        if len(probs_fold) > 0:
            ens_prob = np.mean(list(probs_fold.values()), axis=0)
            ens_pred = (ens_prob >= 0.5).astype(int)
            results["Ensemble_Transfer"]["y_prob"].extend(ens_prob)
            results["Ensemble_Transfer"]["y_pred"].extend(ens_pred)
            
            auc_ens = float(roc_auc_score(y_test, ens_prob)) if len(np.unique(y_test)) > 1 else 0.5
            results["Ensemble_Transfer"]["segs_auc"].append(auc_ens)
    ...
    print("\n=== RESULTADOS GLOBALES CON TRANSFER LEARNING (NIVEL SEGMENTO - LOSO) ===")
    summary_data = []
    for name in results:
    ...
    pd.DataFrame(summary_data).to_csv(RESULTS_DIR / "loso_segment_metrics_transfer.csv", index=False)
    
    # 4. Inferencia Completa para Biomarcador Clínico con el mejor modelo (Ensemble o CNN)
    ...
    # Inferencia con mejor modelo local final (ej: fine-tuning de CNN entrenada final en todo el local)
    # Por simplicidad y robustez, usamos las predicciones acumuladas del bucle LOSO en test_idx
    # Mapeamos los índices de los 1923 segmentos etiquetados a las predicciones LOSO
    all_preds = np.zeros(len(v_subject))
    
    # Re-entrenamos el ensemble en todo el dataset local etiquetado para inferir sobre los no etiquetados
    # Cargar y ajustar escaladores finales
    scaler_final = StandardScaler()
    X_labeled_scaled = scaler_final.fit_transform(X_labeled)
    ...
    if XGB_AVAILABLE and xgb_pretrained is not None:
        ...
        xgb_final.fit(X_labeled, y_labeled, xgb_model=xgb_pretrained.get_booster())
        prob_xgb_all = xgb_final.predict_proba(X_all)[:, 1]
        final_probs.append(prob_xgb_all)
        
    # CNN final
    if TORCH_AVAILABLE:
        cnn_path = RESULTS_DIR / "cnn_pretrained.pt"
        model_final = CAS_CNN()
        if cnn_path.exists():
            model_final.load_state_dict(torch.load(cnn_path, weights_only=True))
        model_final = model_final.to(device)
        
        # Cargar todos los espectrogramas locales
        ...
        if spectrograms_all_path.exists():
            ...
            pos_weight = torch.tensor([np.sum(y_labeled == 0) / np.sum(y_labeled == 1)], dtype=torch.float32).to(device)
            criterion = nn.BCEWithLogitsLoss(pos_weight=pos_weight)
            ...
            model_final.eval()
            X_all_spec_t = torch.tensor(X_all_spec, dtype=torch.float32).unsqueeze(1).to(device)
            with torch.no_grad():
    ...
    if len(final_probs) > 0:
        ...
        print("Error: No hay modelos disponibles para la inferencia completa.")
        return
    ...
    df_res.to_csv(RESULTS_DIR / "clinical_biomarker_results_transfer.csv", index=False)
    ...
    print(f"\nPrueba Mann-Whitney U para Delta CAS con Transfer Learning:")
    ...
    with open(RESULTS_DIR / "statistical_test_transfer.json", "w", encoding="utf-8") as f:
    ...
    plt.title("ROC Curves - Transfer Learning Pipeline (LOSO Segment Level)")
    plt.legend(loc="lower right")
    plt.tight_layout()
    plt.savefig(FIGURES_DIR / "roc_curves_loso_transfer.png", dpi=150)
    plt.close()
    
    print("\nEvaluación de Transfer Learning completada con éxito!")
